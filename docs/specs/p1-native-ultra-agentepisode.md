...
| Increment 4: telemetry | **implemented** | Append-only `<run-dir>/episode_events.jsonl` records run/attempt lifecycle, submission, commit/rejection, Relation outcome counts, readiness/merge events, quality counters, profile, and `usage_source=unavailable` without fabricated App usage or hidden topology. |

The subsequent App-native progression slices implement strict Judge episodes, atomic observable-judgment commit, backend-only geometry/entropy/UCB/allocation progression into Executor grants, complete bounded selected-set blocking Relation inventory, and run-budgeted nonblocking semantic enrichment before a sticky terminal action. Relation candidates, observations, merge applications, readiness completeness, and enrichment spend survive restart. Relation is not a verifier, and discriminator proposals remain unexecuted metadata. Deferred beyond these slices: native Seed and final Synthesis episodes, discriminator research-task scheduling, full production role closure, SDK/App Server transports, visibility into hidden App subagents, telemetry-driven controller tuning, and distributed revisions. The comparison profiles alter only within-episode guidance; they do not change graph semantics or require a minimum subagent count. The normative first-production interpretation is `p1-native-ultra-agentepisode-codex-app-profile.md`.
